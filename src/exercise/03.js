...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('An unexpected error happened: ', error, errorInfo)
  }

  render() {
    if (this.state.hasError) {
      return <h1>An unexpected error happened</h1>
    }
...
function useContext() {
  ...
  if(!context) {
    throw new Error('useTogglr must be used within a <Toggle/>')
  }
